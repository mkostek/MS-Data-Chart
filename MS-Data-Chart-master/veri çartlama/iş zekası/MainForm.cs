		...
		public void fnctn() {
			...
			while(dr.Read())//dr yi dongule
			{
				...
				foreach (DataGridViewColumn column in dataGridView1.Columns) {
					s[k] = dr[column.HeaderText].ToString();
					//başlık değerlerine göre değerlerini al
			...
			{
				if(deger[y]!=0)chartBirinciDerece.Series[0].Points.AddXY(sozce[y],deger[y]);//miktarları göster
				if(ucret[y]!=0)	chartikinciDerece.Series[0].Points.AddXY(sozce[y],ucret[y]);//ücretleri göster
				total+=(int)ucret[y];
			}
